...
<div class="women-product">
    ...
    <ul class="nav nav-tabs">
        ...
        <li><a id="konfirmasi-tab" data-toggle="tab" href="#pembayaran">KONFIRMASI PEMBAYARAN</a></li>
    </ul>
    ...
    <div class="tab-content">
            ...
            <table class="table table-striped table-bordered">
                ...
                <tbody>
                    <?php
                        ...
                        $result=mysqli_query($kon,"select * from pesanan_detail d inner join produk p on p.id=d.kode_produk where nomor_pesanan='$nomor_pesanan' ");
        ...
        <div id="status" class="tab-pane fade">
                <br>
                <div class="table-responsive" style="display: flex; flex-direction: column; height: 100%;">
                        ...
                        <table class="table table-striped table-bordered">
                            ...
                            <tbody>
                                <?php
                                    ...
                                    // Cek status terakhir
                                    $status_result = mysqli_query($kon, "select * from status where nomor_pesanan = '$nomor_pesanan' order by id_status desc");                                    
                                    $latest_status = mysqli_fetch_array($status_result)['status'];
                    ...
                    <div style="text-align: right; margin-top: 10px;">
                        <button type="button" id="complete-order" class="btn btn-info" <?php if ($latest_status == 4) echo 'disabled'; ?>>Pesanan Selesai</button>
                    </div>
                </div>
            </div>
                ...
                <input type="file" name="gambar" class="file" >
                    <div class="input-group my-3">
                        ...
                        <div class="input-group-append">
                                <button style="margin-top: 10px; margin-bottom:10px;" type="button" id="pilih_gambar" class="browse btn btn-dark">Pilih</button>
                        </div>
                    </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#konfirmasi-tab').on('click', function(e) {
            if (!loggedIn) {
                e.preventDefault(); 
                window.location.href = 'index.php?page=login&aut=login';
            }
        });
    });
</script>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        completeOrderBtn.addEventListener('click', function () {
            if (loggedIn) {
                if (confirm("Apakah Anda yakin ingin menyelesaikan pesanan ini?")) {
                    window.location.href = "index.php?page=detail-pesanan&nomor_pesanan=<?php echo $nomor_pesanan; ?>&konfirmasi=1&selesai=1";
                }
            } else {
        ...
        if (<?php echo isset($_GET['konfirmasi']) ? 'true' : 'false'; ?>) {
            konfirmasiTabLink.click();
        }
    });
</script>
...
<?php
    if (isset($_GET['selesai']) && $_GET['selesai'] == 1) {
        ...
        $status = 4;
        $keterangan = "Pesanan telah diselesaikan oleh pelanggan.";

        $query_status = "INSERT INTO status (nomor_pesanan, tanggal, status, keterangan) VALUES ('$nomor_pesanan', '$tanggal', '$status', '$keterangan')";
        mysqli_query($kon, $query_status);
        $query_pesanan = "update pesanan set status='$status' where nomor_pesanan='$nomor_pesanan'";
        mysqli_query($kon, $query_pesanan);

        // echo "<script>alert('Pesanan telah diselesaikan.');</script>";
        echo "<script>window.location.href = 'index.php?page=pesanan-saya&nomor_pesanan=$nomor_pesanan';</script>";
    }
?>