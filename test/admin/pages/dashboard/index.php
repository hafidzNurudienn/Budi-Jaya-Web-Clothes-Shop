...
<section class="content">
    ...
    <div class="col-lg-3 col-xs-6">
    <?php
        $hasil=mysqli_query($kon,"select id_pesanan from pesanan where status='4'");
        if ($hasil){
              ...
              <table class="table table-striped">
                ...
                <?php
                    ...
                    switch ($data['status']){
                        ...
                        case '1' : $status='Pembayaran tertunda';  $label="orange";break;
                        case '2' : $status='Sedang diproses';  $label="blue"; break;
                        case '3' : $status='Dikirim';  $label="black"; break;
                        case '4' : $status='Selesai';  $label="green"; break;
                        case '5' : $status='Dibatalkan'; $label="red"; break;
                        default :  $status='-';
            ...
            <div class="box-body no-padding">
              <table class="table table-striped">
                ...
                <?php
                    ...
                    $semua_pesanan=mysqli_query($kon,"select status_pembayaran FROM pesanan");
                    $jum_semua_pesanan=mysqli_num_rows($semua_pesanan);
                    $hasil=mysqli_query($kon,"select p.nama, sum(qty) as total from pesanan_detail d inner join produk p on p.id=d.kode_produk inner join pesanan n on n.nomor_pesanan=d.nomor_pesanan where status_pembayaran='1' group by p.nama order by total desc");
                    while ($data = mysqli_fetch_array($hasil)):
                    $no++;
