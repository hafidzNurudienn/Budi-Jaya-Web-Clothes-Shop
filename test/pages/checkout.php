...
if (isset($_POST['buat_pesanan'])) {
    ...
    if(!empty($_SESSION["cart"])):
        foreach ($_SESSION["cart"] as $item):
            ...
            $harga=$item["price"];
            $ukuran=$item["size"];
            $simpan_detail = mysqli_query($kon,"insert into pesanan_detail (nomor_pesanan,kode_produk,harga,qty,size) values ('$nomor_pesanan','$kode_produk','$harga','$qty','$ukuran')");
        endforeach;
    endif;
...
if (!isset($_SESSION["cart"])){
    ...
    echo "<a href='../shop.php' class='btn btn-danger' role='button'>Kembali</a>";
    exit;
}
        ...
        <table class="table table-striped table-bordered">
            ...
            <?php
                ...
                foreach ($_SESSION["cart"] as $item):
                    ...
                    $total+=$sub_total;
            ?>
                <input type="hidden" name="kode_produk[]" class="kode_produk" value="<?php echo $item["id"]; ?>"/>
                <tr>
                    <td><?php echo $no; ?></td>
            ...
            ?>
            <tr><td colspan="5">Potongan</td><td> Rp. <?php echo number_format($potongan,0,',','.');?> </td></tr>
            <tr><td colspan="5">Ongkos Kirim</td><td><span id="tampil_ongkir"></span></td></tr>
            <tr><td colspan="5">Total Bayar</td><td><span id="tampil_total_bayar"></span></td></tr>
            </tbody>
        </table>
    </div>
